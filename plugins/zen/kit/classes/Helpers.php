...
class Helpers
{
    ...
    {
        if(!isset($this->input->{$var})) return;
        $var = $this->input->{$var};
        if($method=='trim') return trim($var);
        if($method=='num') return preg_replace("/\D/",'', $var);
    ...
    ### Incline - Склонения существительных ###
