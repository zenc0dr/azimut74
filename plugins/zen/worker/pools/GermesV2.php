...
class GermesV2 extends RiverCrs
{
    public function fillGermesCruises()
    {
        ProcessLog::add('Обработка заездов Germes из SQLite');
        
        $db = new GermesDatabase();
        ...
        foreach ($cruises as $cruise) {
            $germes_ship_id = $cruise['germes_ship_id'];
            $germes_cruise_id = $cruise['germes_cruise_id'];
            $germes_ship = $db->getShipByGermesId($germes_ship_id);
            
            if (!$germes_ship) {
                $this->logError(
                    "Теплоход с ID $germes_ship_id не найден в SQLite",
                    [
                        'germes_ship_id' => $germes_ship_id,
                        'germes_cruise_id' => $germes_cruise_id
                    ]
                );
                ProcessLog::add("Теплоход с ID $germes_ship_id не найден в SQLite");
                $errorsCount++;
                $processedCount++;
            ...
            ProcessLog::add("Обработка заезда germes:$germes_cruise_id (теплоход: {$germes_ship['name']})");
            
            $ship = $this->getMotorship($germes_ship['name'], 'germes_id', $germes_ship_id);
            
            // Проверка исключения теплохода (не считается ошибкой, просто пропускаем)
            if (!$ship) {
                ProcessLog::add("Теплоход {$germes_ship['name']} исключён");
                $processedCount++;
                $this->updateStateFile($processedCount, $totalCruises, $errorsCount, false);
                continue;
            }
            ...
            if (!empty($cruise['date_start'])) {
                try {
                    ...
                    ProcessLog::add("Ошибка парсинга date_start для круиза $germes_cruise_id: " . $e->getMessage());
                }
            }
            ...
            if (!empty($cruise['date_end'])) {
                try {
                    ...
                    ProcessLog::add("Ошибка парсинга date_end для круиза $germes_cruise_id: " . $e->getMessage());
                }
            }

            if (!$dateStart || !$dateEnd) {
                $this->logError(
                    "Отсутствуют даты для круиза germes:$germes_cruise_id",
                    [
                        'germes_cruise_id' => $germes_cruise_id,
                        'germes_ship_id' => $germes_ship_id,
                ...
                ProcessLog::add("Ошибка данных! --- cruise_id:$germes_cruise_id - Отсутствуют даты, заезд игнорирован.");
                $errorsCount++;
                $processedCount++;
                $this->updateStateFile($processedCount, $totalCruises, $errorsCount, false);
                continue;
            ...
            if (!$waybill || empty($waybill) || count($waybill) < 2) {
                $this->logError(
                    "Отсутствует или некорректный маршрут для круиза germes:$germes_cruise_id",
                    [
                        'germes_cruise_id' => $germes_cruise_id,
                        'germes_ship_id' => $germes_ship_id,
                ...
                ProcessLog::add("Ошибка данных! --- cruise_id:$germes_cruise_id - Отсутствует маршрут, заезд игнорирован.");
                $errorsCount++;
                $processedCount++;
                $this->updateStateFile($processedCount, $totalCruises, $errorsCount, false);
                continue;
            ...
            $checkin->desc_1 = $cruise['name'] ?? '';
            $checkin->motorship_id = $ship->id;
            $checkin->active = 1;
            $checkin->eds_code = 'germes';
            $checkin->eds_id = $germes_cruise_id;
            ...
            $pricesImported = $this->importPricesForCruise($checkin->id, $germes_cruise_id, $db, $ship->id);
            
            // Если цены не найдены, деактивируем заезд
            if (!$pricesImported) {
                $this->logError(
                    "Отсутствуют цены для заезда germes:$germes_cruise_id, заезд деактивирован",
                    [
                        'germes_cruise_id' => $germes_cruise_id,
                ...
                ProcessLog::add("Для заезда germes:$germes_cruise_id отсутствуют цены, заезд деактивирован.");
                $checkin->active = 0;
                ...
                $checkin->cachePrices();
                ProcessLog::add("Обработка заезда germes:$germes_cruise_id завершена. Кеш создан.");
            }
        ...
        ProcessLog::add("Обработка всех заездов Germes завершена. Обработано: $processedCount из $totalCruises, ошибок: $errorsCount");
    }
    ...
    private function processWaybillData($waybillData)
    {
        ...
        foreach ($waybill as $index => $point) {
            if (isset($point['town_name'])) {
                $townId = $this->getTownId($point['town_name'], 'germes');
                $result[] = [
                    'town' => $townId,
     ...
     */
    private function importPricesForCruise($checkinId, $germesCruiseId, $db, $shipId)
    {
        // Получаем цены из SQLite
        $prices = $db->getPricesByCruiseId($germesCruiseId);
        ...
        if (!empty($insert_prices)) {
            ...
            ProcessLog::add("Цены для заезда $germesCruiseId: добавлено " . count($insert_prices) . " цен");
            return true; // Цены успешно импортированы
        }
        
        ProcessLog::add("Валидных цен для заезда $germesCruiseId не найдено");
    ...
    {
        try {
            // Получаем stream_id по code='Germes'
            $stream = Stream::where('code', 'Germes')->first();
            
            if (!$stream) {
                // Если stream не найден, просто логируем в ProcessLog
                ProcessLog::add("Не удалось записать ошибку в ErrorLog: stream 'Germes' не найден. Ошибка: $errorMessage");
                return;
            }
            
            $errorLog = new ErrorLog;
            $errorLog->stream_id = $stream->id;
            $errorLog->call = 'GermesV2@fillGermesCruises';
    ...
    {
        $statePath = storage_path('worker/GermesState.yaml');
