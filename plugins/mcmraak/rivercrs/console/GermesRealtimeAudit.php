...
class GermesRealtimeAudit extends Command
{
    protected $name = 'rivercrs:germes-realtime-audit';

    protected $description = 'Проверка realtime-ответов Спутник-Гермес по активным заездам';
    ...
    public function handle()
    {
        ...
        $logDir = storage_path('logs/germes-realtime-audit');
        if (!is_dir($logDir)) {
            mkdir($logDir, 0775, true);
        }

        $logFile = $logDir . '/germes-realtime-audit-' . date('Ymd-His') . '.json';
        ...
        $this->info("Germes realtime аудит: найдено заездов = {$total}");
