...
var RiverCrs = new Vue({
    ...
    mounted: function () {
        ...
        this.startCheckin();
        var $this = this;
        this.sync(null, 'mounted', function (data) {
            $this.DB = data;
    ...
    methods: {
        sync: function (data, action, callback) {
            ...
            var url = '/rivercrs/api/v2/search/'+action;
            var $this = this;
        ...
        toTable: function (way) {
            ...
            var town_id = (this.DB.soft[way.town_id])?this.DB.soft[way.town_id]:way.town_id;
        ...
        formFill: function (callback) {
            ...
            for(var i in this.DB.ways) {
                var town_id = this.DB.ways[i].town_id;
        ...
        yearButtons: function () {
            ...
            for(var i in this.DB.checkins) {
                var checkin_date = this.DB.checkins[i].date;
        ...
        selectActualDate: function () {
            var now = this.nowInSeconds();
            var date_1 = null;
            var date_2 = null;

            // Если есть следующий год
            var nextYear = (this.years.length>1)?this.years[1].value:0;
            var nextYearActive = (nextYear && this.years[1].active)?true:false;
        ...
        openDP: function (date_num) {
            var $this = this;
            var datepicker = $('[name="date-'+date_num+'"]').datepicker({
                //language: $this.L,
                autoClose: true,
            ...
            if(date_num == 1 && this.form.date_1) {
                var ad = new Date(this.dateToSec(this.form.date_1)*1000);
            ...
            if(date_num == 2 && this.form.date_2) {
                var ad = new Date(this.dateToSec(this.form.date_2)*1000);
                datepicker.selectDate(ad);
            }
            datepicker.show();
        },
        ...
        openAirRange:function () {
            var $this = this;
            var datepicker = $('[name="date-range').datepicker({
                autoClose: true,
                onRenderCell: function (date, cellType) {
                    if(!$this.available_dates.length) return;
                    var currentDate = date.getTime();
        ...
        openDR: function () {
            ...
            var calendar = $('input[name="date-range"]');
        ...
        searchFilter: function (callback) {
            ...
            for(var i in this.DB.checkins)
            {
                var checkin = this.DB.checkins[i];
                ...
                // Date OF test
                if(this.form.date_1) {
                    var date_1 = this.dateToSec(this.form.date_1);
                    he = (checkin.date >= date_1)?true:false;
                    if(!he) {
                        //console.log('no date-1');
                        continue;
                    }
                }
                ...
                // Date TO test
                if(this.form.date_2) {
                    var date_2 = this.dateToSec(this.form.date_2);
            ...
            //if(ids.length) this.aimCapture(); // Пока не используется
        ...
        fastTownTest: function (checkin_id, table_num) {
            if(!table_num) {
                ...
                for(var i in this.form.town.value) {
                    var town_id = this.form.town.value[i];
                    if(this.s_table[checkin_id].t[town_id]) return true;

                    //var soft_id = this.DB.soft[town_id];
                    //if(this.s_table[checkin_id].t[soft_id]) return true;
                }
            } else {
                if(!this.form.dest.value) return true;
                if(!this.form.dest.value.length) return true;
                for(var i in this.form.dest.value) {
                    var town_id = this.form.dest.value[i];
                    if(this.s_table[checkin_id].d[town_id]) return true;
                    //var soft_id = this.DB.soft[town_id];
                    //if(this.s_table[checkin_id].d[soft_id]) return true;
                }
            }
            return false;
        },
        ...
        getFilterOptions: function (){
            // ?filter=14-6-28.05.2019-29.05.2019-4-27
            //this.form.town.choose = 63;
...
// Выносное приложение с результатами виджета
if(parseInt($('meta[name="outer_result_container"]').attr('content'))) {
    var RiverResults = new Vue({
        el: '#RiverResults',
        delimiters: ['${','}'],
        data: {
            process: false,
