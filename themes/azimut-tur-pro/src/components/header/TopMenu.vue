<template>
    <div v-click-outside="closeDropdowns" class="top-menu" v-if="width !== null">
        <MenuTree
            v-if="show_desktop_menu"
            :items="items"
            :root-classes="root_classes"
            :item-classes="item_classes"
            :carets="carets"
            :click-outside="click_outside"
        />
        <div class="top-menu_mobile bg-blue-300" v-if="show_mobile_menu">
            <div class="container">
                <div class="py-2 d-flex">
                    <div class="col-auto top-menu_mobile-humburger">
                        <i class="bi bi-list fs-h1 text-white"
                           @click.stop="clickHaburger()"
                           style="cursor:pointer">
                        </i>
                    </div>
                    <div class="col justify-content-center d-flex top-menu_mobile-logo-col">
                        <a href="/" class="top-menu_mobile-mini-logo">
                            <img src="/themes/azimut-tur-pro/assets/images/logo-mini-trans.png" width="50px" alt="Азимут-Тур">
                        </a>
                    </div>

                    <div class="col-auto d-flex align-items-center top-menu_mobile-social">
                        <a href="https://ok.ru/azimuttour" target="_blank" rel="noopener noreferrer" class="top-menu_mobile-social-link" title="Одноклассники">
                            <img src="/themes/azimut-tur-pro/assets/images/social_icons/ok.svg" alt="OK" width="22" height="22">
                        </a>
                        <a href="https://vk.com/azimut.tour" target="_blank" rel="noopener noreferrer" class="top-menu_mobile-social-link" title="ВКонтакте">
                            <img src="/themes/azimut-tur-pro/assets/images/social_icons/vk.svg" alt="VK" width="22" height="22">
                        </a>
                        <a href="https://www.youtube.com/@Азимут-Тур-п4г" target="_blank" rel="noopener noreferrer" class="top-menu_mobile-social-link" title="YouTube">
                            <img src="/themes/azimut-tur-pro/assets/images/social_icons/youtube.svg" alt="YouTube" width="22" height="22">
                        </a>
                        <a href="https://max.ru/join/TCT0dTPCGnHj_y4AjF58dFO8_3ZdV1QER_E9rMeLs7o" target="_blank" rel="noopener noreferrer" class="top-menu_mobile-social-link" title="MAX">
                            <img src="/themes/azimut-tur-pro/assets/images/social_icons/max.svg" alt="MAX" width="22" height="22">
                        </a>
                    </div>

                    <div class="col-auto col-auto d-flex align-items-center justify-content-center">
                        <!--<div class="whats-app-btn kruiz_injector3 only-river-crs" onclick="window.open('https://wa.me/79271149850?text=Меня интересуют речные круизы')"></div>-->
                        <i class="bi bi-telephone-inbound fs-h1 text-white" @click.stop="clickContacts()" style="cursor:pointer"></i>
                    </div>

                </div>
            </div>

            <div v-if="mobile_menu" class="top-menu__mobile-menu bg-blue-300">
                <div class="container">
                    <SliderMenu
                        :items="items"
                        :width="width"
                    />
                </div>
            </div>

            <div v-if="contacts_menu" class="top-menu__contacts-menu bg-primary-100 py-4">
                <div class="container d-flex flex-column align-items-center text-center">
                    <a v-if="!is_rivercrs"
                       href="tel:88452990819"
                       class="text-decoration-none fs-h2 c-primary-200 mb-2">
                        8 (8452) 99 08 19<i style="margin-left: 15px" class="bi bi-telephone-forward"></i>
                    </a>
                    <a href="tel:88001004959" class="text-decoration-none c-primary-200 fs-def fw-bolder">
                        8 (800) 100 49 59
                    </a>
                    <div class="fs-ss mt-1">Звонок бесплатный</div>
                </div>
            </div>
        </div>
    </div>
</template>
<script>
import MenuTree from "./MenuTree";
import SliderMenu from "./SliderMenu";
import ClickOutside from "./directives/ClickOutside";

const MOBILE_BREAKPOINT = 992;
const CONTACTS_CLICK_GUARD_MS = 350;

export default {
    name: 'TopMenu',
    components: {MenuTree, SliderMenu},
    directives: {
        'click-outside': ClickOutside
    },
    created() {
        this.is_rivercrs = location.pathname.includes('russia-river-cruises')
        this.items = JSON.parse($('meta[name="top-menu"]').attr('content'))
        this.bindWidthDetect()
    },
    data() {
        return {
            is_rivercrs: false,
            mobile_menu: false,
            contacts_menu: false,
            last_contacts_click: 0,
            width: typeof window !== 'undefined' ? window.innerWidth : null,
            root_classes: [
                'd-flex justify-content-left',
                'top-menu__dropdown',
            ],
            item_classes: [
                'px-4 py-3  text-white top-menu__top-level fs-s',
                'p-3 text-white top-menu__dropdown-item',
                'text-white level-3'
            ],
            carets: [
                false,
                true
            ],
            click_outside: false
        }
    },
    computed: {
        show_mobile_menu() {
            return this.width <= MOBILE_BREAKPOINT || this.contacts_menu || this.mobile_menu
        },
        show_desktop_menu() {
            return this.width > MOBILE_BREAKPOINT && !this.contacts_menu && !this.mobile_menu
        }
    },
    watch: {
        width(width) {
            if (width > MOBILE_BREAKPOINT) {
                $('body').css('padding-top', 0)
            } else {
                if (!this.contacts_menu) {
                    this.mobile_menu = false
                }
                $('body').css('padding-top', '57px')
            }
        }
    },
    methods: {
        clickHaburger() {
            this.mobile_menu = !this.mobile_menu
            this.contacts_menu = false
            this.switchShadow()
        },
        clickContacts() {
            const now = Date.now()
            if (now - this.last_contacts_click < CONTACTS_CLICK_GUARD_MS) {
                return
            }
            this.last_contacts_click = now
            this.contacts_menu = !this.contacts_menu
            this.mobile_menu = false
            this.switchShadow()
        },
        switchShadow() {
            if (!this.mobile_menu && !this.contacts_menu) {
                if ($('body').hasClass('shadow')) $('body').toggleClass('shadow')
            } else {
                if ($('body').hasClass('shadow') === false) $('body').toggleClass('shadow')
            }
        },
        bindWidthDetect() {
            $(document).ready(() => {
                this.setWidth()
            })

            $(window).resize(() => {
                this.setWidth()
            })
        },
        setWidth() {
            this.width = window.innerWidth
        },
        closeDropdowns() {
            this.click_outside = !this.click_outside
            if (this.contacts_menu || this.mobile_menu) {
                this.contacts_menu = false
                this.mobile_menu = false
                this.switchShadow()
            }
        }
    }
}
</script>
<style>

</style>
